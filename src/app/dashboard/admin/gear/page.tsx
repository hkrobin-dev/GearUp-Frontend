...
export default function AdminGearPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-900">All Gear Listings</h1>
      <p className="mt-1 text-slate-500">Every gear item across all providers.</p>
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan={5} className="py-12">
                  <EmptyState icon={Boxes} title="No gear listed" />
                </td>
              </tr>
            ) : (
              gear.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="px-4 py-3 text-slate-500">{item.provider?.name}</td>
