...
export default function PayOrderPage() {
  ...
  if (!order) {
    return <p className="text-slate-500">Order not found.</p>;
  }

  const alreadyPaid = order.status !== "PLACED" && order.status !== "CONFIRMED";

  return (
    <div className="mx-auto max-w-lg">
      <h1 className="text-2xl font-bold text-slate-900">Complete Payment</h1>
      <p className="mt-1 text-slate-500">Review your order before proceeding to Stripe.</p>
      ...
      {alreadyPaid ? (
        <div className="mt-6 rounded-lg bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
          This order has already been paid or moved past payment. Current status:{" "}
        ...
        >
          <CreditCard className="h-5 w-5" /> Pay {formatCurrency(order.totalAmount)} with Stripe
        </Button>
      )}
      ...
      <Button variant="ghost" className="mt-2 w-full" onClick={() => router.push("/dashboard/customer/orders")}>
        Back to orders
      </Button>
    </div>
  );
